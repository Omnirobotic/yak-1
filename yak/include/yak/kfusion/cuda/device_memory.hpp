...
namespace kfusion {
namespace cuda {
/** \brief Error handler. All GPU functions from this subsystem call the function to report an error. For internal use
 * only */
...
public:
    ...
    /** \brief Allocates internal buffer in GPU memory. If internal buffer was created before the function recreates it
     * with new size. If new and old sizes are equal it does nothing. \param sizeBytes_arg: buffer size
     * */
    ...
    /** \brief Uploads data to internal buffer in GPU memory. It calls create() inside to ensure that intenal buffer size
     * is enough. \param host_ptr_arg: pointer to buffer to upload \param sizeBytes_arg: buffer size
     * */
...
public:
    ...
    /** \brief Allocates internal buffer in GPU memory. If internal buffer was created before the function recreates it
     * with new size. If new and old sizes are equal it does nothing. \param ptr_arg: number of rows to allocate \param
    ...
    /** \brief Uploads data to internal buffer in GPU memory. It calls create() inside to ensure that intenal buffer size
     * is enough. \param host_ptr_arg: pointer to host buffer to upload \param host_step_arg: stride between two
     * consecutive rows in bytes for host buffer \param rows_arg: number of rows to upload \param sizeBytes_arg: width of
     * host buffer in bytes
     * */
    ...
    /** \brief Returns stride between two consecutive rows in bytes for internal buffer. Step is stored always and
     * everywhere in bytes!!! */
    size_t step() const;

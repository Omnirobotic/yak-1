...
inline int convertSMVer2Cores(int major, int minor)
{
    ...
    SMtoCores gpuArchCoresPerSM[] =
            {{0x10, 8}, {0x11, 8}, {0x12, 8}, {0x13, 8}, {0x20, 32}, {0x21, 48}, {0x30, 192}, {0x35, 192}, {0x50, 128}, {0x52, 128}, {-1, -1}};
...
void kf::cuda::printCudaDeviceInfo(int device)
{
    ...
    const char* computeMode[] = {
            ...
            "Exclusive (only one host thread in one process is able to use ::cudaSetDevice() with "
            "this device)",
            "Prohibited (no host thread can use ::cudaSetDevice() with this device)",
            "Exclusive Process (many threads in one process is able to use ::cudaSetDevice() with "
            "this device)",
            "Unknown",
            NULL};
    ...
    for (int dev = beg; dev < end; ++dev)
    {
        ...
        // This is not available in the CUDA Runtime API, so we make the necessary calls the driver API to support this for
        // output
